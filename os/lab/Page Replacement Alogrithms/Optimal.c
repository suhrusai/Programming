...
#include<stdlib.h>
#include<limits.h>
void optimal(int pages[],int no_of_pages,int pagetable[],int len_page_table,int current_index){
  int i=0,j;
  int *recently_used=(int *)malloc(sizeof(recently_used)*len_page_table);
  for(i=0;i<len_page_table;i++)
  {
      //Checking the least recent use of a frame in the page table further down the line
      for(j=current_index;j<no_of_pages;j++){
...
void logic(int pages[],int no_of_pages,int pagetable[],int len_page_table){
    ...
    for(i=0;i<no_of_pages;i++){
        ...
        if(j==len_page_table){
            optimal(pages,no_of_pages,pagetable,len_page_table,i);
            pagefault++;
            if(i>len_page_table)
                pagereplacement++;
        }
    }
    printf("\npagefaults: %d page replacements: %d",pagefault,pagereplacement);
}
void main(){
